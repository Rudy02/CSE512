# bestfirst_astar_search_heap.py
...
def ASTAR_SEARCH_FCT(start,target,GOAL_FCT,SUCCESSOR_FCT,EVAL_FCT,\
  ...
  open = [[0, start, [start], 0]]  # [(EVAL_FCT), (PUZZ), (PATH), (DEPTH)]
  closed = []
  steps = 0
  while open != []:
    nxt = open[0]
    ...
    for x in succ:
        ...
        #newnode = [x,addpath(nxtpath,x),xcost+nxtdpth+1,nxtdpth+1]
...
'''
'''
print "\nBest-first search for RED DONKEy:"
show_puzz(START)
for i in range(1):
  BEST_FIRST_SEARCH_FCT(START, None, donkey_goal, donkey_successors,\
                        donkey_eval, donkey_compact)
'''

print "\nA* search in for RED DONKEY:"
show_puzz(START)
for i in range(1):
  ASTAR_SEARCH_FCT(START, None, donkey_goal, donkey_successors,\
                   donkey_eval, donkey_compact)
